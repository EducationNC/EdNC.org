...
class WPNA_Admin_Facebook extends WPNA_Admin_Base implements WPNA_Admin_Interface {
	...
	public function hooks() {
		...
		if ( wpna_switch_to_boolean( wpna_get_option( 'fbia_enable' ) ) ) {
			...
			// Add options to the post Publish meta.
			add_action( 'post_submitbox_misc_actions', array( $this, 'add_publish_meta_options' ), 10, 1 );
	...
	public function register_post_meta() {
		register_meta( 'post', $this->post_status_input_name, array(
			'sanitize_callback' => 'sanitize_text_field', // Note: We also sanitize with our own filters.
			'type'              => 'string',
			'description'       => esc_html__( 'Instant Article status.', 'wp-native-articles' ),
			'show_in_rest'      => true,
			'single'            => true,
		) );
	...
	/**
	 * Add the 'Instant Article' option to the Publish Meta box.
	 *
	 * @access public
	 * @param WP_Post $post The WP Post object.
	 * @return void
	 */
	public function add_publish_meta_options( $post ) {
		// Check we want to add it to this post type.
		if ( ! in_array( get_post_type( $post ), wpna_allowed_post_types(), true ) ) {
			return;
		}

		// Check the current user has the current permissions.
		...
		// Get the FBIA status for this post.
		$fbia_status = get_post_meta( $post->ID, '_wpna_fbia_status', true );

		// If there isn't one set then default to publish.
		// The API will import as draft if it's set.
		if ( empty( $fbia_status ) ) {
		...
		// Check the post type.
		if ( ! in_array( get_post_type( $post ), wpna_allowed_post_types(), true ) ) {
			$disable_warning = esc_html__( 'Instant Articles are not enabled for this post type.', 'wp-native-articles' );
			$fbia_status     = 'draft';
		}

		// Don't publish posts with password protection.
		if ( post_password_required( $post ) ) {
			$disable_warning = esc_html__( 'Instant Articles cannot be used with password protected posts.', 'wp-native-articles' );
			$fbia_status     = 'draft';
		}

		// Don't publish privately published posts.
		if ( 'private' === get_post_status( $post ) ) {
			$disable_warning = esc_html__( 'Instant Articles cannot be used with privately published posts.', 'wp-native-articles' );
			$fbia_status     = 'draft';
		}

		// Translatable labels.
		$labels = array(
			'published' => esc_html__( 'Published', 'wp-native-articles' ),
			'draft'     => esc_html__( 'Draft', 'wp-native-articles' ),
		);
		?>
		<div class="misc-pub-section misc-fbia-status">

			<span class="wpna-fbia-icon"></span>
			<?php esc_html_e( 'Instant Article:', 'wp-native-articles' ); ?>
			<strong class="wpna-fbia-status-text"><?php echo ( isset( $labels[ $fbia_status ] ) ? esc_html( $labels[ $fbia_status ] ) : esc_html__( 'Unknown', 'wp-native-articles' ) ); ?></strong>

			<a href="#wpna_fbia_status" class="wpna-edit-fbia-status hide-if-no-js" role="button">
				<span aria-hidden="true"><?php esc_html_e( 'Edit', 'wp-native-articles' ); ?></span>
				<span class="screen-reader-text"><?php esc_html_e( 'Edit Status', 'wp-native-articles' ); ?></span>
			</a>

			<div id="fbia-status-select" class="hide-if-js">
				<?php if ( $disable_warning ) : ?>
					<div class="inline notice notice-warning notice-alt">
						<p><?php echo esc_html( $disable_warning ); ?></p>
					</div>
				<?php else : ?>
					<label for="wpna_fbia_status" class="screen-reader-text"><?php esc_html( 'Set Facebook Instant Article status', 'wp-native-articles' ); ?></label>
					<select name="wpna_options[<?php echo esc_attr( $this->post_status_input_name ); ?>]" id="wpna_fbia_status">
						<option value="published"<?php selected( $fbia_status, 'published' ); ?>><?php echo esc_html( $labels['published'] ); ?></option>
						<option value="draft"<?php selected( $fbia_status, 'draft' ); ?>><?php echo esc_html( $labels['draft'] ); ?></option>
					</select>
					<a href="#wpna_fbia_status" class="wpna-save-fbia-status hide-if-no-js button"><?php esc_html_e( 'OK', 'wp-native-articles' ); ?></a>
				<?php endif; ?>

				<a href="#wpna_fbia_status" class="wpna-cancel-fbia-status hide-if-no-js button-cancel"><?php esc_html_e( 'Cancel', 'wp-native-articles' ); ?></a>
			</div>
