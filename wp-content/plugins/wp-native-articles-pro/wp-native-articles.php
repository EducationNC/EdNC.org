<?php
/**
 * Plugin Name: WP Native Articles Premium
...
if ( ! class_exists( 'WP_Native_Articles_Pro' ) ) :

	/**
	 * Main WP_Native_Articles_Pro Class
	 *
	 * Code layout inspired by Affiliate_WP
	 *
	 * @since  1.5.0
	 */
	final class WP_Native_Articles_Pro {
		/** Singleton *************************************************************/

		/**
		 * WP_Native_Articles_Pro instance.
		 *
		 * @access private
		 * @since  1.5.0
		 * @var    WP_Native_Articles_Pro The one true WP_Native_Articles_Pro
		 */
		private static $instance;

		/**
		 * The version number of WP_Native_Articles_Pro.
		 *
		 * @access private
		 * @since  1.5.0
		 * @var    string
		 ...
		 * @access public
		 * @since  1.5.0
		 * @var    WPNA_Admin_General_Licensing
		 */
		public $admin_general_licensing;
		...
		/**
		 * Main WP_Native_Articles_Pro Instance
		 *
		 * Insures that only one instance of WP_Native_Articles_Pro exists in memory at any one
		 * time. Also prevents needing to define globals all over the place.
		 *
		 * @since  1.5.0
		 * @static
		 * @staticvar array $instance
		 * @return WP_Native_Articles_Pro
		 */
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof WP_Native_Articles_Pro ) ) {
				self::$instance = new WP_Native_Articles_Pro();
		...
		private function setup_constants() {
			...
			if ( ! defined( 'WPNA_ITEM_NAME' ) ) {
				define( 'WPNA_ITEM_NAME', 'WP Native Articles Pro' );
			}

			// Flag for Premium. Doesn't unluck anyting, just prevents
			// some checks for premium features and shows some notices.
			if ( ! defined( 'WPNA_PREMIUM' ) ) {
				define( 'WPNA_PREMIUM', true );
			}
		}
		...
		private function includes() {
			...
			require WPNA_BASE_PATH . '/includes/lib/EDD_SL_Plugin_Updater.php';
			require WPNA_BASE_PATH . '/includes/lib/facebook.php';
			...
			require WPNA_BASE_PATH . '/includes/class-facebook-api.php';
			...
			require WPNA_BASE_PATH . '/includes/admin/general/class-admin-general-licensing.php';
			...
			require WPNA_BASE_PATH . '/includes/placements/functions-placements.php';
			require WPNA_BASE_PATH . '/includes/placements/actions-placements.php';
			...
			require WPNA_BASE_PATH . '/includes/placements/class-placement.php';
		...
		public function setup_objects() {
			...
			self::$instance->admin_general_licensing          = new WPNA_Admin_General_Licensing();
...
if ( ! function_exists( 'wpna' ) ) :
	...
	/**
	 * The main function responsible for returning the one true WP_Native_Articles_Pro
	 ...
	 *
	 * @since  1.3.5
	 * @return WP_Native_Articles_Pro The one true WP_Native_Articles_Pro Instance
	 */
	function wpna() {
		global $wpna;
		...
		if ( ! $wpna ) {
			...
			$wpna = WP_Native_Articles_Pro::instance();
		}
 ...
 *
 * To be shown if this plugin is trying to be activated over the free one.
 *
 * @since  1.0.0
 * @return void
 */
function wpna_disable_free_plugin_check() {

	if ( is_plugin_active( 'wp-native-articles/wp-native-articles.php' ) ) {
		...
		wp_die(
			esc_html__( 'Please disable the Free version before activating the Pro version.', 'wp-native-articles' ),
...
// If the Free plugin is active register the notice function to both the plugin
// activation hook and admin_init (incase it was activated in an obscure manner).
register_activation_hook( __FILE__, 'wpna_disable_free_plugin_check' );
add_action( 'admin_init', 'wpna_disable_free_plugin_check', 1, 0 );
