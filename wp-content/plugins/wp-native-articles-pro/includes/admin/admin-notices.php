...
/**
 * Show a prompt if the licence key is missing or inactive for premium users.
 *
 * @since 1.4.0
 * @return void
 */
function wpna_license_notices() {

	// Don't bother if it's a network site.
	if ( is_multisite() && is_plugin_active_for_network( plugin_basename( WPNA_BASE_PATH . '/wp-native-articles.php' ) ) ) {
		return;
	}

	// If they've recently dismissed the notice then don't bother.
	if ( false !== get_transient( 'wpna_license_notice' ) ) {
		return;
	}
	...
	if ( 'expired' === $status ) {
		...
		$notice_query_args['wpna-notice'] = 'expired_license';

		$class = 'notice notice-error';

		$message .= sprintf(
			wp_kses(
				// translators: Placeholder is the URL to the account.
				__( 'Your licence for WP Native Articles has expired. Please <a target="_blank" href="%s">renew your licence</a> to continue receiving automatic updates.', 'wp-native-articles' ),
				array(
					'a' => array(
						'href'   => array(),
						'target' => array(),
		...
		$message .= '<p><a href="' . wp_nonce_url( add_query_arg( $notice_query_args ), 'wpna-dismiss-notice', 'wpna-dismiss-notice-nonce' ) . '">' . _x( 'Dismiss Notice', 'License', 'wp-native-articles' ) . '</a>';

	} elseif ( 'valid' !== $status ) {

		$notice_query_args['wpna-notice'] = 'invalid_license';

		$class = 'notice notice-info';

		$message .= sprintf(
			// translators: Placeholder is the URL to the licencing page.
			__( 'Please <a href="%s">enter and activate</a> your license key for WP Native Articles to enable automatic updates.', 'wp-native-articles' ),
			esc_url( add_query_arg( array( 'page' => 'wpna_general' ), admin_url( 'admin.php' ) ) )
		) . '</p>';
		$message .= '<p><a href="' . wp_nonce_url( add_query_arg( $notice_query_args ), 'wpna-dismiss-notice', 'wpna-dismiss-notice-nonce' ) . '">' . _x( 'Dismiss Notice', 'License', 'wp-native-articles' ) . '</a>';
	}
...
add_action( 'admin_notices', 'wpna_license_notices', 10, 0 );
