...
$appearances = wp_get_post_terms(get_the_id(), 'appearance');
  ...
  $app_hide[] = array_search('Digest Spotlight', array_column($appearances, 'name'));
...
if ($column) {
  ...
  $cat_under_column=$column[0]->name;
  ?>
	<!--
	<span class="label column">
		<a href="<?php// echo $link; ?>"><?php //echo $column[0]->name; ?></a>
	</span> 
	-->
	<h1 class="h6 entry-title text-purple">
		<a href="<?php echo $link; ?>" ><?php echo $column[0]->name; ?></a>
	</h1>
  <?php
}
...
// Category label
if ($category && $cat_under_column!="Reach NC Polls") {
...
if ($category && $cat_under_column=="Reach NC Polls") {
  foreach ($category as $key=>$value) {
    if (!in_array($key, $cats_hide)) {
      $link = get_category_link($value['term_id']);
      echo '<span class="label"><a href="' . $link . '" class="disabled">' . $value['cat_name'] . '</a></span> ';
    }
  }
}
