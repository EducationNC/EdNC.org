...
class socialNetworkShareCount{
    ...
    public function getTwitterShares(){
        $api = file_get_contents( 'https://opensharecount.com/count.json?url=' . $this->shareUrl );
        $count = json_decode( $api );
    ...
    public function getPinterestShares(){
        $api = file_get_contents( 'https://api.pinterest.com/v1/urls/count.json?callback%20&url=' . $this->shareUrl );
