<div class="callout accordion" id="accordion-years" role="tablist" aria-multiselectable="true">
  ...
  $new_array = array_merge($wp_query->query, ['fields' => 'ids', 'posts_per_page' => -1, 'post_type' => ['post', 'map', 'edtalk','reach-nc-poll']]);
  ...
  $prepared_query = $wpdb->prepare("SELECT DISTINCT DAY (post_date) AS day, MONTH( post_date ) AS month , YEAR( post_date ) AS year, COUNT( id ) as post_count FROM $wpdb->posts WHERE id IN (%s) GROUP BY day, month, year ORDER BY post_date DESC", join(',', $results));
  $query = str_replace(array('(\'', '\')'), array('(', ')'), $prepared_query);
  $days = $wpdb->get_results($query);

  // Determine which years and months need to be expanded on page load
  if (isset($_GET['date'])) {
    $expanded_year = date('Y', strtotime($_GET['date']));
    $expanded_month = date('m', strtotime($_GET['date']));
  } else {
  ...
  foreach($days as $day) :
    ...
    <li class="archive-day">
      <a href="?date=<?php echo $day->year; ?>/<?php echo date("m", mktime(0, 0, 0, $day->month, 1, $day->year)); ?>/<?php echo date("d", mktime(0, 0, 0, $day->month, $day->day, $day->year)); ?>">
...
<p><a class="btn btn-default" href="<?php echo get_category_feed_link($cat_id); ?>feed/"><span class="icon-rss"></span> Category RSS Feed</a></p>