...
if (function_exists('shortcode_ui_register_for_shortcode')) :
   ...
   */
	...
	/**
	 * Creates the correct IA output for the parallax-image shortcode.
	 *
	 * @param  array $attr Shortcode attributes.
	 * @return string
	 */
	function wpna_do_custom_parallax_image_shortcode( $attr ) {
		...
		// If no image is set then bail early.
		if ( ! $image_id ) {
			return $output;
		}
		...
		$output .= sprintf( '<img src="%s"/>', $image_src[0] );

		// Check if there's a caption.
		if ( ! empty( $floating_text ) ) :
			$output .= '<figcaption class="op-vertical-center, op-center"><h1>';
			$output .= esc_html( $floating_text );
			$output .= '</h1></figcaption>';
		endif;
		...
		return '<figure data-mode="fullscreen" data-feedback="fb:likes, fb:comments">' . $shortcode_key . '</figure>' . PHP_EOL;
	}

	/**
	 * Override the shortcode callback function when in IA.
	 *
	 * @param array  $override_tags Shortcode tags being overridden.
	 * @param string $content       Current post content.
	 * @return array
	 */
	function wpna_add_custom_parallax_image_shortcode_callback( $override_tags, $content ) {
		$override_tags['parallax-image'] = __NAMESPACE__ . '\\wpna_do_custom_parallax_image_shortcode';
		return $override_tags;
	}
	add_filter( 'wpna_facebook_article_setup_wrap_shortcodes_override_tags', __NAMESPACE__ . '\\wpna_add_custom_parallax_image_shortcode_callback', 10, 2 );
