...
class WPNA_Admin extends WPNA_Admin_Base {
	...
	public function hooks() {
		add_action( 'admin_init',            array( $this, 'initialize_updater' ), 0, 0 );
	 ...
	 *
	 * Uses the EDD updater. Uses the license key to check for updates
	 *
	 * @since 1.0.0
	 *
	 * @access public
	...
	public function initialize_updater() {
		...
		// Get the correct license key.
		$license_key = get_site_option( 'wpna_license_key' );

		// Setup the updater.
		$edd_updater = new WPNA_EDD_SL_Plugin_Updater( WPNA_STORE_URL, WPNA_BASE_PATH . '/wp-native-articles.php',
			array(
				'version'   => WPNA_VERSION, // current version number.
				'license'   => trim( $license_key ), // license key.
				'author'    => 'WPArtisan', // author of this plugin.
				'item_name' => WPNA_ITEM_NAME, // name of this plugin.
				'item_id'   => 48,
				'url'       => home_url(),
				'beta'      => false,
			)
