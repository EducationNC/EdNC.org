...
if ( ! function_exists( 'wpna_activate_license' ) ) :

	/**
	 * Activates a plugin license.
	 *
	 * Pings the main server and tries to activate the license.
	 * Returns with the status or an arror message.
	 * Only deals with remote activation, remember to update the local option.
	 *
	 * @since 1.0.0
	 *
	 * @param  string $license The key to validate.
	 * @return array e.g.
	 *
	 * array(
	 ...
	 * )
	 */
	function wpna_activate_license( $license ) {

		// data to send in our API request.
		$api_params = array(
			'edd_action' => 'activate_license',
			'license'    => $license,
			'item_name'  => rawurlencode( WPNA_ITEM_NAME ),
			'url'        => home_url(),
		);

		// Call the custom API.
		$response = wp_remote_post( WPNA_STORE_URL,
			array(
				'timeout'   => 15,
				'sslverify' => false,
				'body'      => $api_params,
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'wp-native-articles' );
			}
		} else {

			$license_data = json_decode( wp_remote_retrieve_body( $response ) );

			// It was a valid request but the license is invalid.
			if ( false === $license_data->success ) {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							// translators: Placeholder is the date their licence expires.
							esc_html__( 'Your license key expired on %s.', 'wp-native-articles' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'wp-native-articles' );
						break;

					case 'missing':
						$message = esc_html__( 'Invalid license.', 'wp-native-articles' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'wp-native-articles' );
						break;

					case 'item_name_mismatch':
						// translators: Placeholder is the name of the Plugin.
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'wp-native-articles' ), WPNA_ITEM_NAME );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'wp-native-articles' );
						break;
...
if ( ! function_exists( 'wpna_deactivate_license' ) ) :

	/**
	 * Deactivates a plugin license.
	 *
	 * Pings the main server and tries to deactivate the license.
	 * Returns with the status or an arror message.
	 * Only deals with remote deactivation, remember to update the local option.
	 *
	 * @since 1.0.0
	 *
	 * @param  string $license The key to deactivate.
	 * @return array e.g.
	 *
	 * array(
	 *  'state' => valid or invalid,
	 ...
	 * )
	 */
	function wpna_deactivate_license( $license ) {
...
if ( ! function_exists( 'wpna_license_check' ) ) :

	/**
	 * Check whether the licence is valid or not.
	 *
	 * @since 1.4.0
	 * @param bool $force Bypass cache and force a new check.
	 * @return string
	 */
	function wpna_license_check( $force = false ) {

		// Get the old license checked.
		$status = get_transient( 'wpna_license_check' );

		// No old license status, check it again.
		if ( ! $status || $force ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				set_transient( 'wpna_license_check', $response, 3 * HOUR_IN_SECONDS );
			...
			// 'license' key is actually status.
			update_option( 'wpna_license_status', $license_data->license );

			set_transient( 'wpna_license_check', $license_data->license, DAY_IN_SECONDS );
...
if ( ! function_exists( 'wpna_add_facebook_notification' ) ) :

	/**
	 * Store a Facebook error for display later on.
	 *
	 * Takes a WP_Error object and stores it for access later.
	 *
	 * @since 1.1.6
	 *
	 * @param  WP_Error $error Stores an error in a globla array for access later.
	 * @return void
	 */
	function wpna_add_facebook_notification( WP_Error $error ) {

		if ( ! isset( $GLOBALS['wpna_facebook_errors'] ) ) {
			$GLOBALS['wpna_facebook_errors'] = array();
		}

		$GLOBALS['wpna_facebook_errors'][] = $error;
	}
endif;


if ( ! function_exists( 'wpna_get_facebook_notifications' ) ) :

	/**
	 * Retrieves all Facebook errors.
	 *
	 * Returns an array of WP_Errors.
	 *
	 * @since 1.1.6
	 *
	 * @return null|array
	 */
	function wpna_get_facebook_notifications() {
		$errors = null;

		if ( ! empty( $GLOBALS['wpna_facebook_errors'] ) ) {
			$errors = $GLOBALS['wpna_facebook_errors'];
		}
		 ...
		 * @since 1.1.6
		 *
		 * @param array|null $errors Array of WP_Errors.
		 */
		$errors = apply_filters( 'wpna_get_facebook_notifications', $errors );
